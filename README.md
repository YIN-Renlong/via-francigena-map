...
### Phase 11: Polish, Typography Unification, and Deployment Architecture (Post-Review Iteration. 23 May, 2026)
...
6. **Modernizing the Data-Ink Ratio (Flat Design Transition):** To align the topographical dashboard with contemporary, high-end data-visualization standards, the Chart.js rendering engine was heavily refactored. Legacy "Web 2.0" aesthetics—such as heavy drop-shadows and frosted-glass borders—were eradicated in favor of a crisp "Flat Design" UI. Raw, jagged GPS telemetry is now elegantly smoothed using a Bézier spline curve (tension: 0.15). Furthermore, the background grid was rebuilt following Edward Tufte's strict "data-ink" principles: the harsh vertical axis spine and tick notches were removed entirely, leaving only a whisper-quiet, 6%-opacity dashed horizontal grid. This preserves vital scaling context while evoking the aesthetic of architectural drafting paper, perfectly matching the documentary tone of the archive.

### Phase 12: Final Academic Review & The WordPress Portal Architecture (23 May, 2026)

Following a final peer-review session with academic advisors (Prof. Dr. Frederik Truyen), a concluding layer of polish was applied to bridge the standalone GitHub repository with the parent WordPress CMS. This phase focused on advanced responsive design, data legibility, and seamless cross-origin deployment.

1. **The Editorial Portal (Fluid CSS Architecture):** To transition users gracefully from the standard WordPress site into the immersive MapLibre environment, a custom HTML/CSS gateway (.vfa-entrance) was engineered. This section utilizes advanced modern CSS, including CSS variables (--vfa-paper, --vfa-ink), clamp() for fluid, mathematically scaling typography, and multi-layered linear gradients to simulate the texture of archival drafting paper. A custom SVG decal and an animated, shadow-casting CTA button establish a premium, editorial aesthetic akin to high-end digital journalism (e.g., *The New York Times*).
2. **Responsive "Grid Breaking" & Readability:** Addressing academic feedback regarding text readability, the intersection of narrative prose and photographic evidence was refined. On mobile viewports (max-width: 800px), strict media queries enforce wider margins, ensuring the text column has ample breathing room and never overlaps the imagery. However, on large desktop displays, the slight negative margins (margin-left: -60px;) were deliberately retained. In digital editorial design, this intentional "breaking of the grid" physically anchors the image to the text, replicating the sophisticated, tactile layout of print museum exhibition catalogs.
3. **Data Legibility (The Topographical Legend):** To ensure the telemetry dashboard is instantly legible without external explanation, a localized legend was injected directly into the Chart.js canvas container. Utilizing the same "frosted glass" (backdrop-filter: blur) aesthetic as the interactive waypoints, it decodes the route's color-mapped speed telemetry (Blue <12km/h, Yellow <22km/h, Orange <32km/h, Red Sprint) without consuming valuable viewport space.
4. **Zero-Friction CI/CD (The Custom WP Plugin):** Standard iframe embeds often suffer from browser throttling, scroll-hijacking, and aggressive caching. To solve this, a bespoke PHP plugin (via-francigena-updater.php) was developed for the WordPress backend. It acts as a 1-click CI/CD pipeline. When triggered by an admin, it bypasses GitHub's CDN cache (?nocache=time()), downloads the latest repository ZIP, unpacks it directly into the server's /map directory, and applies strict file-type blacklists (ignoring .py backend scripts). This allows the interactive documentary to be hosted *natively* within the CMS, guaranteeing maximum rendering performance.
5. **Curation and Pagination:** To prevent "scrolling fatigue," the project scope was deliberately curated. Rather than dumping the entire 16-day expedition into a single DOM instance, the first 4 days were packaged as a standalone, complete narrative chapter. This honors the limits of human attention in digital humanities interfaces, leaving the architectural framework in place for future paginated expansions.
6. **Academic Acknowledgments:** Formal credits were integrated into both the documentary metadata and the parent CMS footer, acknowledging the invaluable mentorship of Prof. Dr. Frederik Truyen (KU Leuven) in shaping the project's digital heritage methodology.

### Phase 13: The 3D Globe Experiment and the Limits of Spatial Scale (Post-Review UX Audit. 23 May, 2026)

In an attempt to replicate the high-end, cinematic minimap aesthetics of Esri ArcGIS StoryMaps, an experimental branch of the project temporarily replaced the bottom-right MapLibre minimap with a true WebGL 3D spinning globe. This phase served as a profound lesson in the conflict between aesthetic ambition and functional spatial storytelling.

1. **The CSS "Glass Orb" Illusion:** The initial attempt maintained the MapLibre engine but applied advanced CSS pseudo-elements (::after, radial-gradient, and inset box-shadows) to fake 3D specular lighting and a glassy drop-shadow. However, because MapLibre v3.x lacks native globe projection, the map remained flat inside a circular clipping mask, breaking the planetary illusion.
2. **The Three.js WebGL Engine:** To achieve a true 3D sphere, the secondary MapLibre instance was stripped out entirely and replaced with a lightweight Three.js engine. A sphere geometry was constructed and textured with the *Esri World Physical Map*. To replicate StoryMaps' realistic lighting, a custom JavaScript pixel-scanning algorithm was written to dynamically generate a specular map (detecting blue/cyan pixels to make the oceans shiny while leaving landmasses matte). Furthermore, Scrollama's onStepEnter coordinates were converted into Quaternions, allowing the 3D globe to cinematically roll (slerp) to the active GPS location.
3. **The Resolution Bottleneck:** While visually stunning from a "space" perspective, the implementation quickly revealed the limitations of static 3D texturing. A global WebGL sphere relies on a single 4K image texture. When the 3D camera was zoomed in close enough to make Italy legible, the texture suffered from severe pixelation (blurring). Unlike modern map tile servers that dynamically stream high-resolution SVG vectors at close range, the 3D sphere degraded the closer the user looked.
4. **The Spatial Paradox & UX Reversion:** Ultimately, the 3D globe introduced a fatal narrative flaw: **The Spatial Paradox of Scale**. A 1,000-kilometer bicycle expedition represents an immense human effort, but on a global planetary scale, it is microscopically small. If the globe was zoomed out to look beautiful, a grueling 80km daily ride registered as a barely perceptible vibration on the screen. If the globe was zoomed in to show the route, the planetary illusion was lost and the texture blurred. Coupled with the heavy bandwidth cost of loading 4K textures and running a secondary WebGL rendering loop, the 3D minimap actively harmed the documentary's performance and utility.
5. **Conclusion:** The Three.js globe was deliberately abandoned, and the project reverted to the unified, lightweight 2D MapLibre minimap. By synchronizing the minimap to the primary "Modern Antique" vector style, the interface regained its crisp archival aesthetic, eliminated the performance bottleneck, and ensured that the physical progress of the expedition remained functionally legible to the reader at all times. Function and narrative clarity were prioritized over cinematic excess.

### Phase 14: Mobile Geometry, The "Spotlight Effect," and Visual Hierarchy  (23 May, 2026)

During final cross-device testing, two critical UX friction points were identified regarding how the interface handled photographic integrity on restricted screens, and how persistent UI elements competed with narrative immersion.

1. **Resolving the "Spotlight Effect" (Minimap UI Clash):** While the circular MapLibre minimap provided excellent geographic context, it created a severe visual hierarchy clash during the cinematic immersive-left and cover layouts. The bright, high-contrast UI element acted as a visual "spotlight," aggressively pulling the reader's eye away from the dark, moody photographic evidence. To resolve this without deleting the feature, a "Ghost UI" logic was injected into the Scrollama onStepEnter event. When a full-screen photo layout triggers, the JavaScript dynamically applies a .minimap-ghost CSS class, dropping the map's opacity and stripping its saturation. It retreats into the shadows, respecting the photography, but remains available if the user actively seeks it.
2. **The Mobile Parallax Bug (background-attachment):** A known rendering engine bug in mobile browsers (particularly iOS Safari) causes background-attachment: fixed to calculate its dimensions based on the height of the *entire DOM document* rather than the viewport. This caused the full-screen parallax photographs to stretch and zoom by over 500%. Aggressive @media (max-width: 800px) overrides were implemented to force background-attachment: scroll, stabilizing the mobile rendering engine and returning the images to their native scale.
3. **The "Geometry Paradox" (DSLR vs. Mobile Viewports):** Because the expedition was shot on a DSLR (producing wide 3:2 landscape rectangles) and mobile phones use portrait viewports (tall 9:16 rectangles), the standard CSS background-size: cover rule forced severe horizontal cropping. This destroyed the original photographic compositions.
4. **The Print Editorial Solution:** To prioritize documentary integrity over modern web trends, the mobile CSS was rewritten to use background-size: contain with no-repeat. Instead of cropping the image to fit the screen, the browser now renders the 100% uncut landscape photograph at the top of the mobile viewport. The resulting negative space below the image is filled with a stark #111111 background, seamlessly bridging the gap to the text box. This transforms the mobile experience from a standard "web layout" into a classic, high-end "museum exhibition catalog" aesthetic, ensuring the photography is viewed exactly as the author captured it.
